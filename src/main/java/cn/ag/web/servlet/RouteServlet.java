...
public class RouteServlet extends BaseServlet {
  ...
  private   FavoriteService favoriteService = new FavoriteServiceImpl();
     ...
     * 猜你喜欢 推荐旅游页面展示功能
     */
    public void indexRecommendRoute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int count = 4;
     ...
     * 主题旅游展示
     */
    public void indexThemeRoute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int count = 4;
        List<Route> route = routeService.findindexThemeRoute(count);
        writeValue(route, response);
//      List<List<Route>> lists = routeService.findindexThemeRoute(count);
//      writeValue(lists, response);
    }

    //index part3 大模块 国内+国外+推荐（？）
    ...
    public void findrouteindexrecommend(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // List<Category> categoryList = categoryService.findAll();
        List<Route> findrouteindexguowai = routeService.findrouteindexguowai();
        writeValue(findrouteindexguowai, response);
    }
